"""Analysis script for 15_cluster_demo."""
...
set_theme()
OUT_DIR = Path("fig/15_cluster_demo")
...
def _extract_cluster_k(row, *, info: dict | None = None) -> float:
    info = (row.get("info", {}) or {}) if info is None else info
    for value in (row.get("cluster_k"), info.get("cluster_k")):
        if value is not None:
            try:
                return float(int(value))
            except (TypeError, ValueError):
    ...
    return pd.Series(
        {
            ...
            "cluster_k": _extract_cluster_k(row, info=info),
...
def _common_row_fields(row, *, info, train_iters, mid_pred, train_alpha, task_shape_idx, task_shape_tag, train_max_n_demos, cluster_k):
    train_demo_ranked = _extract_train_demo_ranked(row, info=info)
    n_params = _compute_row_n_params(info)
...
def _save_aggregates_for_mid_pred_and_train_iters(
    *,
    ...
    out_dir = out_root / f"mid{int(mid_pred)}" / f"train_iters_{int(train_iters)}" / f"train_max_demos_{int(train_max_n_demos)}" / f"cluster_k_{int(cluster_k)}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    final_slice = final_df.loc[
        ...
        & (final_df["cluster_k"] == float(cluster_k))
    ].copy()
    role_slice = role_eval_demo_df.loc[
        ...
        & (role_eval_demo_df["cluster_k"] == float(cluster_k))
    ].copy()
    needle_slice = role_eval_needle_df.loc[
        ...
        & (role_eval_needle_df["cluster_k"] == float(cluster_k))
    ...
    demo_all_slice = role_eval_demo_all_df.loc[
        ...
        & (role_eval_demo_all_df["cluster_k"] == float(cluster_k))
    ...
    groupby_cols = ["train_iters", "model_family", "mid_pred", "train_alpha", "train_max_n_demos", "cluster_k"]
    ...
    if not role_slice.empty:
        ...
        role_groupby = [
            ...
            "train_max_n_demos", "cluster_k", "train_demo_ranked", "eval_role", "eval_max_n_demos", "eval_alpha",
        ]
        ...
        for eval_ranked_val in eval_ranked_values:
                ...
                for eval_role in ("train", "eval"):
                    _plot_role_metric_group(
                        ...
                        title=(
                            ...
                            f"(mid{int(mid_pred)}, ti={int(train_iters)}, eval_{tag}, k={cluster_k})"
                        ),
            ...
            if "forward_flops_at_eval_seq" in compute_sub.columns:
                _plot_accuracy_vs_compute_metric(
                    ...
                    title=f"Accuracy vs Inference FLOPs (mid{int(mid_pred)}, ti={int(train_iters)}, eval_{tag}, k={cluster_k})",
            ...
            if "activation_memory_at_eval_seq" in compute_sub.columns:
                _plot_accuracy_vs_compute_metric(
                    ...
                    title=f"Accuracy vs Inference Activation Memory (mid{int(mid_pred)}, ti={int(train_iters)}, eval_{tag}, k={cluster_k})",
        ...
        for eval_ranked_val in needle_ranked_values:
            ...
            if "forward_flops_at_eval_seq" in compute_sub.columns:
                _plot_accuracy_vs_compute_metric(
                    ...
                    x_col="forward_flops_at_eval_seq",
                    x_label="Forward FLOPs at eval seq len (log)",
                    title=f"Needle: Accuracy vs Inference FLOPs (mid{int(mid_pred)}, ti={int(train_iters)}, needle_{tag}, k={cluster_k})",
            ...
            if "activation_memory_at_eval_seq" in compute_sub.columns:
                _plot_accuracy_vs_compute_metric(
                    ...
                    x_col="activation_memory_at_eval_seq",
                    x_label="Inference activation memory (bytes, log)",
                    title=f"Needle: Accuracy vs Inference Activation Memory (mid{int(mid_pred)}, ti={int(train_iters)}, needle_{tag}, k={cluster_k})",
    ...
    if not demo_all_slice.empty:
        ...
        _plot_demo_all_comparison(
            ...
            title=f"demo_all rollout_success_rate (mid{int(mid_pred)}, ti={int(train_iters)}, k={cluster_k})",
        )


# <codecell>
df = collate_dfs("remote/15_cluster_demo/set", show_progress=True)
if len(df) == 0:
    raise ValueError("No results found in remote/15_cluster_demo/set")
...
sort_cols = ["model_family", "mid_pred", "train_alpha", "train_max_n_demos", "cluster_k"]
