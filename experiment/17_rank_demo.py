"""Analysis script for 17_rank_demo (full_rank beta sweep)."""
...
def _extract_train_beta(row, *, info: dict | None = None) -> float:
    info = (row.get("info", {}) or {}) if info is None else info
    for value in (row.get("train_beta"), info.get("train_beta")):
    ...
    return pd.Series(
        {
            ...
            "train_beta": _extract_train_beta(row, info=info),
...
def _common_row_fields(row, *, info, train_iters, mid_pred, train_beta, task_shape_idx, task_shape_tag, train_max_n_demos):
    n_params = _compute_row_n_params(info)
...
def _explode_3level_metrics(
    df: pd.DataFrame,
    *,
    metrics_col: str,
    eval_type: str | None = None,
) -> pd.DataFrame:
    """Explode role -> n_demos -> eval_beta -> metrics."""
    rows = []
    for _, row in df.iterrows():
        ...
        base = _common_row_fields(
            ...
            train_beta=train_beta, task_shape_idx=task_shape_idx,
            task_shape_tag=task_shape_tag, train_max_n_demos=train_max_n_demos,
        )
        for role, by_demo in by_role.items():
            for eval_demo, by_beta in (by_demo or {}).items():
                for eval_beta_val, metrics in (by_beta or {}).items():
                    metrics = metrics or {}
...
def _explode_role_eval_demo_rows(df: pd.DataFrame) -> pd.DataFrame:
    return _explode_3level_metrics(df, metrics_col="metrics_by_role_eval_demo")


def _explode_role_eval_needle_rows(df: pd.DataFrame) -> pd.DataFrame:
    return _explode_3level_metrics(
...
def _plot_role_metric_group(
    *,
    ...
    plot_df["facet_label"] = (
        "train_beta=" + plot_df["train_beta"].apply(
            lambda x: "inf" if x == float('inf') else str(x)
        )
    )
    plot_df["eval_beta_label"] = plot_df["eval_beta"].apply(
        lambda x: "inf" if x == float('inf') else str(x)
    )

    long_df = plot_df[
        ["model_family", "train_beta", "facet_label", "eval_beta",
         "eval_beta_label", "eval_max_n_demos", *available]
    ].melt(
        id_vars=["model_family", "train_beta", "facet_label", "eval_beta",
                 "eval_beta_label", "eval_max_n_demos"],
    ...
    eval_beta_order = sorted(long_df["eval_beta_label"].unique(), key=lambda s: float(s))
    palette = dict(
        zip(eval_beta_order, sns.color_palette("flare", n_colors=len(eval_beta_order)))
    )
    ...
    if legend is not None:
        legend.set_title("eval_beta / family")
...
def _plot_rollout_success_rate_tiled(
    *,
    ...
    """Tiled figure showing rollout_success_rate per train_beta."""
    plot_df = role_eval_demo_df.loc[role_eval_demo_df["eval_role"] == str(eval_role)].copy()
    ...
    facet_order = (
        plot_df[["train_beta", "facet_label"]]
        .drop_duplicates()
        .sort_values("train_beta")["facet_label"]
        .tolist()
    )
    eval_beta_order = sorted(plot_df["eval_beta_label"].unique(), key=lambda s: float(s))
    palette = dict(
        zip(eval_beta_order, sns.color_palette("flare", n_colors=len(eval_beta_order)))
    )
...
def _plot_heatmap(
    *,
    ...
    ax.set_xlabel("eval_beta")
    ax.set_ylabel("train_beta")
    fig.tight_layout()
...
def _plot_demo_all_comparison(
    *,
    ...
    plot_df["facet_label"] = (
        plot_df["model_family"] + " train_beta=" + plot_df["train_beta"].apply(
            lambda x: "inf" if x == float('inf') else str(x)
        )
    )
    plot_df = plot_df.dropna(subset=["rollout_success_rate"])
...
def _plot_accuracy_vs_compute_metric(
    *, eval_df: pd.DataFrame, x_col: str, x_label: str, title: str, out_path: Path,
    ...
    # Sort facets by numeric train_beta
    facet_order = (
        plot_df[["train_beta", "facet_label"]]
        .drop_duplicates()
        .sort_values("train_beta")["facet_label"]
    ...
    eval_beta_order = sorted(
        plot_df["eval_beta_label"].unique(), key=lambda s: float(s)
    )
    palette = dict(
        zip(eval_beta_order, sns.color_palette("flare", n_colors=len(eval_beta_order)))
    )
...
def _save_aggregates_for_mid_pred_and_train_iters(
    *,
    ...
    groupby_cols = ["train_iters", "model_family", "mid_pred", "train_beta", "train_max_n_demos"]
    final_agg = (
        final_slice.groupby(
            groupby_cols,
            as_index=False,
        ...
        .sort_values(["model_family", "mid_pred", "train_beta"])
    ...
    if not role_slice.empty:
        ...
        role_groupby = [
            "train_iters", "model_family", "mid_pred", "train_beta",
            "train_max_n_demos", "eval_role", "eval_max_n_demos", "eval_beta",
        ]
        ...
        role_agg = (
            ...
            .sort_values(
                ["eval_role", "model_family", "train_beta", "eval_beta", "eval_max_n_demos"]
            )
            .reset_index(drop=True)
        )
        ...
        for group in ROLE_METRIC_PLOT_GROUPS:
            for eval_role in ("train", "eval"):
                _plot_role_metric_group(
                    ...
                    out_path=out_dir / f"eval_{eval_role}_{group['filename']}",
                    title=(
                        f"{str(eval_role).capitalize()}: {str(group['title'])} "
                        f"(mid{int(mid_pred)}, ti={int(train_iters)})"
                    ),
        ...
        for eval_role in ("train", "eval"):
            _plot_rollout_success_rate_tiled(
                role_eval_demo_df=role_agg,
                eval_role=eval_role,
                out_path=out_dir / f"rollout_success_rate_tiled_{eval_role}.png",
                title=(
        ...
        if "forward_flops_at_eval_seq" in role_slice.columns:
            _plot_accuracy_vs_compute_metric(
                eval_df=role_slice,
                x_col="forward_flops_at_eval_seq",
                x_label="Forward FLOPs at eval seq len (log)",
                title=f"Accuracy vs Inference FLOPs (mid{int(mid_pred)}, ti={int(train_iters)})",
                out_path=out_dir / "eval_accuracy_vs_inference_flops.png",
            )
        if "activation_memory_at_eval_seq" in role_slice.columns:
            _plot_accuracy_vs_compute_metric(
                eval_df=role_slice,
                x_col="activation_memory_at_eval_seq",
                x_label="Inference activation memory (bytes, log)",
                title=f"Accuracy vs Inference Activation Memory (mid{int(mid_pred)}, ti={int(train_iters)})",
                out_path=out_dir / "eval_accuracy_vs_activation_memory.png",
            )

    # --- Needle eval plots (commented out — needle eval disabled) ---
    # if not needle_slice.empty:
    #     ...

    # --- demo_all comparison (commented out — demo_all eval disabled) ---
    # if not demo_all_slice.empty:
    #     ...


# <codecell>
df = collate_dfs("remote/17_rank_demo/set", show_progress=True)
if len(df) == 0:
    raise ValueError("No results found in remote/17_rank_demo/set")
...
sort_cols = ["model_family", "mid_pred", "train_beta", "train_max_n_demos"]
...
def _plot_compute_efficiency(*, role_eval_demo_df, out_dir):
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    for beta_label in eval_beta_order:
        grp = plot_df.loc[plot_df["eval_beta_label"] == beta_label]
        for family, fgrp in grp.groupby("model_family"):
            ...
            ax.scatter(
                ...
                color=palette[beta_label], marker=marker, alpha=0.5, s=20,
            )
    # Custom legend entries
    from matplotlib.lines import Line2D
    beta_handles = [
        Line2D([0], [0], marker="o", color=palette[b], linestyle="", markersize=5, label=b)
        for b in eval_beta_order
    ]
    family_handles = [
    ...
    ax.legend(
        handles=beta_handles + family_handles,
        title="eval_beta / family",
        fontsize="small",
