"""Cluster demo distribution sweep for the depth3_fresh_icl fresh-rule split."""
...
TRAIN_ITERS_SWEEP = [10_000]
TEST_EVERY = 1000
TEST_ITERS = 2
ROLLOUT_EXAMPLES_PER_ROLE = 25
...
def _fresh_icl_config(
    ...
    *,
    ...
    cluster_unselected_rank: int | None = CLUSTER_UNSELECTED_RANK,
) -> dict:
...
for bundle in TASK_SHAPE_BUNDLES:
    ...
    for n_layers, (n_hidden, n_heads), lr, pos_encoding, use_swiglu, train_iters, train_alpha, train_max_n_demos, train_demo_ranked, cluster_k in itertools.product(
        TRANSFORMER_LAYERS,
        ...
        case_name = (
            f"15_cluster_demo_{bundle['task_shape_tag']}_transformer_"
    ...
    for n_layers, (n_hidden, n_heads), d_state, d_conv, scan_chunk_len, lr, train_iters, train_alpha, train_max_n_demos, train_demo_ranked, cluster_k in itertools.product(
        ...
        case_name = (
            f"15_cluster_demo_{bundle['task_shape_tag']}_mamba2_bonsai_"
            f"l{int(n_layers)}_h{int(n_hidden)}_heads{int(n_heads)}_"
...
for case in tqdm(all_cases, desc="cases", leave=True):
    ...
    cluster_k = int(case.info["cluster_k"])
    ...
    # --- Needle eval (zipf_per_rule_headless + include_oracle) ---
    n_needle_jobs = int(
        len(EVAL_ROLES) * len(EVAL_MAX_N_DEMOS_SWEEP)
        * len(EVAL_ALPHA_SWEEP) * len(EVAL_DEMO_RANKED_SWEEP)
    )
