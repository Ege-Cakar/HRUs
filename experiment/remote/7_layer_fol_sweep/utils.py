"""Layer-FOL-sweep-specific utility helpers."""
...
from task.layer_gen.util.fol_rule_bank import FOLAtom
...
def infer_src_layer_from_prompt_tokens(
    ...
    *,
    ...
    for ant in sequent.ants:
        if not isinstance(ant, FOLAtom):
            raise ValueError(f"Expected FOLAtom antecedents, got {type(ant).__name__}")
        match = _PRED_RE.fullmatch(ant.predicate)
        if match is None:
            raise ValueError(f"Unsupported layered predicate name: {ant.predicate}")
