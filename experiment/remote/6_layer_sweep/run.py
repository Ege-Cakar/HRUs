"""Architecture sweep on LayerTask distance generalization in online mode."""
...
### START TEST CONFIGS
# TRAIN_MAX_DISTANCES = [3]
# EVAL_DISTANCES = [1, 2, 3, 20]
# BATCH_SIZE = 16
# TRAIN_ITERS = 100
# TEST_EVERY = 50
# TEST_ITERS = 1
# EVAL_ITERS_PER_DISTANCE = 1
# ROLLOUT_EXAMPLES_PER_DISTANCE = 16
# RUN_SPLIT = 1
...
for train_max_distance in TRAIN_MAX_DISTANCES:
    ...
    for n_layers, (n_hidden, n_heads), lr, pos_encoding, use_swiglu in itertools.product(
        ...
        case_name = f"6_layer_sweep_transformer_k{int(train_max_distance):02d}"
    ...
    for n_layers, (n_hidden, n_heads), d_state, d_conv, scan_chunk_len, lr in itertools.product(
        ...
        case_name = f"6_layer_sweep_mamba2_k{int(train_max_distance):02d}"
        train_args["wandb_cfg"] = _make_case_wandb_cfg(
            case_name=case_name,
            model_config=config,
            train_args=train_args,
    ...
    for n_layers, n_hidden, n_channels, lr in itertools.product(
        ...
        case_name = f"6_layer_sweep_mixer_completion_k{int(train_max_distance):02d}"
        train_args["wandb_cfg"] = _make_case_wandb_cfg(
            case_name=case_name,
            model_config=config,
            train_args=train_args,
