"""Alpha-beta demo distribution sweep for the depth3_fresh_icl fresh-rule split."""
...
SWEEP_TRAIN_ALPHA = [0, 2, 10]
EVAL_ALPHA_SWEEP = [0, 1, 2, 3, 4, 10]

SWEEP_TRAIN_BETA = [0.0, 0.5, 1.0, float('inf')]
EVAL_BETA_SWEEP = [0.0, 0.25, 0.5, 1.0, 2.0, float('inf')]
...
# SWEEP_TRAIN_BETA = [0.0, float('inf')]
# EVAL_BETA_SWEEP = [0.0, float('inf')]
# SWEEP_MID_PRED = [32]
# SWEEP_TASK_SHAPES = [
#     {
...
def _evaluate_role_for_demo(
    ...
    *,
    ...
    eval_alpha: float,
    eval_demo_ranking_beta: float = float('inf'),
    demo_distribution: str = DEMO_DISTRIBUTION,
    include_oracle: bool = False,
    demo_all: bool = False,
    batch_size: int | None = None,
    ...
    eval_task = _make_layer_task(
        ...
        demo_ranking_beta=float(eval_demo_ranking_beta),
        demo_all=bool(demo_all),
        include_oracle=bool(include_oracle),
    ...
    agg.update(
        {
            ...
            "eval_demo_ranking_beta": float(eval_demo_ranking_beta),
...
def _metric_by_role_demo(
    ...
    *,
    ...
    beta_metrics = (alpha_metrics or {}).get(float(eval_beta), {})
    value = beta_metrics.get(metric_name)
    if value is None:
        return float("nan")
    return float(value)
...
for bundle in TASK_SHAPE_BUNDLES:
    ...
    for n_layers, (n_hidden, n_heads), lr, pos_encoding, use_swiglu, train_iters, train_alpha, train_max_n_demos, train_beta in itertools.product(
        TRANSFORMER_LAYERS,
        ...
        train_task = _make_layer_task(
            ...
            demo_ranking_beta=float(train_beta),
        ...
        case_name = (
            f"16_ab_demo_{bundle['task_shape_tag']}_transformer_"
    ...
    for n_layers, (n_hidden, n_heads), d_state, d_conv, scan_chunk_len, lr, train_iters, train_alpha, train_max_n_demos, train_beta in itertools.product(
        ...
        case_name = (
            f"16_ab_demo_{bundle['task_shape_tag']}_mamba2_bonsai_"
            f"l{int(n_layers)}_h{int(n_hidden)}_heads{int(n_heads)}_"
...
for case in tqdm(all_cases, desc="cases", leave=True):
    ...
    train_beta = float(case.info["train_beta"])
    ...
    n_standard_jobs = int(
        ...
        * len(EVAL_ALPHA_SWEEP) * len(EVAL_BETA_SWEEP)
    )
    eval_job_bar = tqdm(
    ...
    for role in EVAL_ROLES:
        role_metrics: dict[int, dict[float, dict[float, dict]]] = {}
        for eval_max_n_demos in EVAL_MAX_N_DEMOS_SWEEP:
            ...
            alpha_metrics: dict[float, dict[float, dict]] = {}
            for eval_alpha in EVAL_ALPHA_SWEEP:
                beta_metrics: dict[float, dict] = {}
                for eval_beta in EVAL_BETA_SWEEP:
                    ...
                    beta_metrics[float(eval_beta)] = _evaluate_role_for_demo(
                        case.optimizer,
                        task_shape=bundle["task_shape"],
                        role=str(role),
                        ...
                        eval_max_n_demos=int(eval_max_n_demos),
                        eval_alpha=float(eval_alpha),
                        eval_demo_ranking_beta=float(eval_beta),
                        demo_distribution=DEMO_DISTRIBUTION,
                ...
                alpha_metrics[float(eval_alpha)] = beta_metrics
    ...
    for role in EVAL_ROLES:
        role_metrics_n: dict[int, dict[float, dict[float, dict]]] = {}
        for eval_max_n_demos in EVAL_MAX_N_DEMOS_SWEEP:
            this_dims = bundle["dims_by_n_demos"][int(eval_max_n_demos)]
            this_eval_n_seq = int(_ceil_pow2_int(int(this_dims["n_seq_ar"])))
            this_batch_size, this_n_iters = _estimate_batch_and_iters(
            ...
            alpha_metrics_n: dict[float, dict[float, dict]] = {}
            for eval_alpha in EVAL_ALPHA_SWEEP:
                beta_metrics_n: dict[float, dict] = {}
                for eval_beta in EVAL_BETA_SWEEP:
                    ...
                    beta_metrics_n[float(eval_beta)] = _evaluate_role_for_demo(
                        case.optimizer,
                        task_shape=bundle["task_shape"],
                        role=str(role),
                        tokenizer=bundle["tokenizer"],
                    ...
                    needle_bar.update(1)
                alpha_metrics_n[float(eval_alpha)] = beta_metrics_n
    ...
    row = {
        ...
        "selection_eval_beta": float(selection_eval_beta),
