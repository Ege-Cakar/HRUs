"""Full-rank beta sweep for the depth3_fresh_icl fresh-rule split."""
...
TRAIN_MIN_N_DEMOS = 1
SWEEP_TRAIN_MAX_N_DEMOS = [64, 224]
...
def _evaluate_role_for_demo(
    ...
    *,
    ...
    _beta_seed = 9999 if eval_demo_ranking_beta == float('inf') else int(round(eval_demo_ranking_beta * 100)) % 10_000
    ...
    agg.update(
        {
            ...
            "eval_beta": float(eval_demo_ranking_beta),
...
def _metric_by_role_demo(
    ...
    *,
    ...
    beta_metrics = (demo_metrics or {}).get(float(eval_beta), {})
...
for bundle in TASK_SHAPE_BUNDLES:
    ...
    for n_layers, (n_hidden, n_heads), lr, pos_encoding, use_swiglu, train_iters, train_max_n_demos, train_beta in itertools.product(
        TRANSFORMER_LAYERS,
        ...
        case_name = (
            f"17_rank_demo_{bundle['task_shape_tag']}_transformer_"
    ...
    for n_layers, (n_hidden, n_heads), d_state, d_conv, scan_chunk_len, lr, train_iters, train_max_n_demos, train_beta in itertools.product(
        ...
        case_name = (
            f"17_rank_demo_{bundle['task_shape_tag']}_mamba2_bonsai_"
            f"l{int(n_layers)}_h{int(n_hidden)}_heads{int(n_heads)}_"
            f"ds{int(d_state)}_lr{_lr_tag(lr)}_"
            f"d{int(train_max_n_demos)}_"
            f"{_beta_tag(train_beta)}_"
            f"ga{int(train_grad_accum_steps)}_ti{int(train_iters)}"
        )
...
for case in tqdm(all_cases, desc="cases", leave=True):
    ...
    for role in EVAL_ROLES:
        role_metrics: dict[int, dict[float, dict]] = {}
        for eval_max_n_demos in EVAL_MAX_N_DEMOS_SWEEP:
            ...
            role_metrics[int(eval_max_n_demos)] = beta_metrics
        metrics_by_role_eval_demo[str(role)] = role_metrics
    eval_job_bar.close()

    # --- Needle eval (commented out for initial exploration) ---
    # n_needle_jobs = int(
    #     len(EVAL_ROLES) * len(EVAL_MAX_N_DEMOS_SWEEP)
    #     * len(EVAL_BETA_SWEEP)
    # )
    ...
    # for role in EVAL_ROLES:
    #     role_metrics_n: dict[int, dict[float, dict]] = {}
    #     for eval_max_n_demos in EVAL_MAX_N_DEMOS_SWEEP:
    #         this_dims = bundle["dims_by_n_demos"][int(eval_max_n_demos)]
    #         this_eval_n_seq = int(_ceil_pow2_int(int(this_dims["n_seq_ar"])))
    #         this_batch_size, this_n_iters = _estimate_batch_and_iters(
    ...
    #             )
    #             needle_bar.update(1)
    #         role_metrics_n[int(eval_max_n_demos)] = beta_metrics_n
    #     metrics_by_role_eval_needle[str(role)] = role_metrics_n
    # needle_bar.close()
    metrics_by_role_eval_needle = {}

    # --- demo_all eval (commented out for initial exploration) ---
