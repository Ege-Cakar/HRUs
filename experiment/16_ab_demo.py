"""Analysis script for 16_ab_demo."""
...
def _beta_file_tag(beta) -> str:
    if beta == float('inf') or (isinstance(beta, str) and beta == 'inf'):
        return "binf"
    return "b" + str(float(beta)).replace(".", "p")


def _beta_display(beta) -> str:
    if pd.isna(beta):
        return "n/a"
    if beta == float('inf'):
        return "inf"
    return f"{float(beta):.2g}"
    ...
    for value in (row.get("train_beta"), info.get("train_beta")):
        if value is not None:
            try:
                ...
                continue
    # Backward compat: fall back to demo_ranked boolean
    for value in (row.get("train_demo_ranked"), info.get("train_demo_ranked")):
        if value is not None:
            return float('inf') if bool(value) else 0.0
    return float('inf')
...
def _explode_4level_metrics(
    ...
    *,
    ...
    """Explode role -> n_demos -> eval_alpha -> eval_beta -> metrics."""
    rows = []
    for _, row in df.iterrows():
        ...
        for role, by_demo in by_role.items():
            for eval_demo, by_alpha in (by_demo or {}).items():
                for eval_alpha, by_beta in (by_alpha or {}).items():
                    # Detect legacy bool keys vs new float keys
                    if isinstance(by_beta, dict) and any(
                        isinstance(k, bool) for k in by_beta
                    ):
                        # Legacy 4-level format with bool keys
                        for eval_ranked, metrics in (by_beta or {}).items():
                            metrics = metrics or {}
                            eval_top = metrics.get("eval_top_seq_lens", [])
                            beta_val = float('inf') if bool(eval_ranked) else 0.0
                            out = {
                                **base,
                                "eval_role": str(role),
                            ...
                            rows.append(out)
                    elif isinstance(by_beta, dict) and not any(
                        isinstance(k, (bool, int, float)) and not isinstance(k, bool) for k in by_beta
                        if isinstance(k, (int, float))
                    ) and all(isinstance(k, str) for k in by_beta):
                        # Legacy 3-level format: by_beta IS the metrics dict
                        metrics = by_beta or {}
                        eval_top = metrics.get("eval_top_seq_lens", [])
                        out = {
                            **base,
                        ...
                        # New format: float keys for beta
...
def _save_aggregates_for_mid_pred_and_train_iters(
    *,
    ...
    if "train_beta" in final_slice.columns:
        groupby_cols.append("train_beta")
    final_agg = (
        final_slice.groupby(
            groupby_cols,
            as_index=False,
    ...
    # --- Standard eval plots (separate files per eval_beta) ---
    if not role_slice.empty:
        ...
        role_groupby = [
            ...
            "train_max_n_demos", "train_beta", "eval_role", "eval_max_n_demos", "eval_alpha",
        ]
        if "eval_beta" in role_slice.columns:
            role_groupby.append("eval_beta")
        ...
        eval_beta_values = (
            sorted(role_agg["eval_beta"].dropna().unique().tolist())
            if "eval_beta" in role_agg.columns
            else [None]
        )
        for eval_beta_val in eval_beta_values:
            if eval_beta_val is not None:
                sub = role_agg.loc[role_agg["eval_beta"] == eval_beta_val].copy()
                tag = _beta_file_tag(eval_beta_val)
            else:
                sub = role_agg.copy()
                tag = "all"
            ...
            if eval_beta_val is not None and "eval_beta" in role_slice.columns:
                compute_sub = role_slice.loc[role_slice["eval_beta"] == eval_beta_val]
            else:
    ...
    if not needle_slice.empty:
        ...
        if "eval_beta" in needle_slice.columns:
            needle_groupby.append("eval_beta")
        ...
        needle_beta_values = (
            sorted(needle_agg["eval_beta"].dropna().unique().tolist())
            if "eval_beta" in needle_agg.columns
            else [None]
        )
        for eval_beta_val in needle_beta_values:
            if eval_beta_val is not None:
                sub = needle_agg.loc[needle_agg["eval_beta"] == eval_beta_val].copy()
                tag = _beta_file_tag(eval_beta_val)
            else:
                sub = needle_agg.copy()
                tag = "all"
            ...
            # --- Compute metric scatter plots (unaggregated) ---
            if eval_beta_val is not None and "eval_beta" in needle_slice.columns:
                compute_sub = needle_slice.loc[needle_slice["eval_beta"] == eval_beta_val]
            else:
...
# <codecell>
df = collate_dfs("remote/16_ab_demo/set", show_progress=True)
if len(df) == 0:
    raise ValueError("No results found in remote/16_ab_demo/set")
...
if "train_beta" in final_df.columns:
    sort_cols.append("train_beta")
