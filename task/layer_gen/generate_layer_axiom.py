"""Generate ArrayRecord datasets for layered axiom-sequence tasks."""
...
@dataclass
class _FirstStepStats:
    max_token: int = 0
    max_seq: int = 0
    max_prompt_seq: int = 0
    max_target_seq: int = 0
...
@dataclass
class _DistanceStateFirstStep:
    writer: ArrayRecordShardWriter
    examples: int = 0
    stats: _FirstStepStats = field(default_factory=_FirstStepStats)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate layered axiom-sequence datasets.",
    )
    ...
    parser.add_argument(
        "--objective",
        type=str,
        choices=("autoreg", "first_step", "both"),
        default="both",
    )
...
def _objective_roots(out_dir: Path, objective: str) -> dict[str, Path]:
    if objective == "autoreg":
        return {"autoreg": out_dir}
    if objective == "first_step":
        return {"first_step": out_dir}
    return {
        "autoreg": out_dir / "autoreg",
        "first_step": out_dir / "first_step",
    }
...
def _update_first_step_stats(stats: _FirstStepStats, prompt: list[int], target: list[int]) -> None:
    if prompt:
        stats.max_token = max(stats.max_token, max(prompt))
    if target:
        stats.max_token = max(stats.max_token, max(target))

    stats.max_prompt_seq = max(stats.max_prompt_seq, len(prompt))
    stats.max_target_seq = max(stats.max_target_seq, len(target))
    stats.max_seq = max(stats.max_seq, len(prompt), len(target))
...
def _write_objective_metadata_autoreg(
    *,
    root: Path,
    ...
    metadata = {
        ...
        "format": "arrayrecord_layer_axiom_autoreg",
        "objective": "autoreg",
        "seed": seed,
...
def _write_objective_metadata_first_step(
    *,
    root: Path,
    ...
    for distance, state in states.items():
        ...
        distances[str(distance)] = {
            ...
            "stats": {
                ...
                "max_seq": int(st.max_seq),
                "max_prompt_seq": int(st.max_prompt_seq),
                "max_target_seq": int(st.max_target_seq),
            },
        }
    ...
    overall = {
        ...
        "max_seq": int(max(st.max_seq for st in stats_list)),
        "max_prompt_seq": int(max(st.max_prompt_seq for st in stats_list)),
        "max_target_seq": int(max(st.max_target_seq for st in stats_list)),
    }

    metadata = {
        "created_at": created_at,
        "format": "arrayrecord_layer_axiom_first_step",
...
def main() -> None:
    ...
    roots = _objective_roots(args.out_dir, args.objective)

    autoreg_states: dict[int, _DistanceStateAutoreg] = {}
    first_step_states: dict[int, _DistanceStateFirstStep] = {}

    if "autoreg" in roots:
        root = roots["autoreg"]
        for distance in distances:
            writer = ArrayRecordShardWriter(
                out_dir=root / f"distance_{distance:03d}",
    ...
    if "first_step" in roots:
        ...
        for distance in distances:
            ...
            first_step_states[distance] = _DistanceStateFirstStep(writer=writer)
    ...
    with tqdm(total=total_target, desc="generate layered axiom", leave=True) as bar:
        pending = 0
        for distance in distances:
            for _ in range(args.examples_per_distance):
                ...
                if "first_step" in roots:
                    state = first_step_states[distance]
                    rule0 = sampled.step_rules[0]
                    ...
                    payload = pickle.dumps(
                        {
                            "distance": int(sampled.distance),
                            "start_layer": int(sampled.start_layer),
                            "src_layer": int(src_layer0),
                            "goal_atom": sampled.goal_atom,
                            "prompt": np.asarray(prompt0, dtype=np.int32),
                            "target_first": np.asarray(target0, dtype=np.int32),
                            "statement_text": rule0.statement_text,
                        },
                        protocol=5,
                    )
                    state.writer.write(payload)
                    _update_first_step_stats(state.stats, prompt0, target0)
                    state.examples += 1
    ...
    for state in autoreg_states.values():
        state.writer.close()
    for state in first_step_states.values():
        state.writer.close()

    rule_bank_rel = str(rule_bank_out.resolve())
    ...
    root_meta = {
        "created_at": created_at,
        "format": "arrayrecord_layer_axiom",
        "seed": int(args.seed),
        "objective": args.objective,
        "objectives": sorted(list(roots.keys())),
        "distance_range": [int(args.min_distance), int(args.max_distance)],
