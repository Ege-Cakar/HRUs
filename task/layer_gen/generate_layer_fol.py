...
try:
    from task.layer_fol.ipc_util import (
        ipc_read_pickle_frame as _ipc_read_pickle_frame,
        ipc_write_pickle_frame as _ipc_write_pickle_frame,
    )
except ImportError:
    # Fallback for direct execution (e.g., python generate_layer_fol.py)
    ...
    def _ipc_write_pickle_frame(stream, payload_obj) -> None:
        payload = _pickle.dumps(payload_obj, protocol=5)
