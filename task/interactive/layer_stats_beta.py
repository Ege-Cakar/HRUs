# <codecell>
"""Interactive stats for mid-layer predicate count vs Plackett-Luce beta difficulty.

Computes two probabilities across a grid of (mid_pred × beta × n_demos):

1. **P(demo good)**: P(at least one full_rank-sampled in-context demo is rank-1,
...
This is the ``full_rank`` analog of ``layer_stats_mid_pred.py``, which sweeps
over Zipf ``alpha``.  Here we sweep over the Plackett-Luce ``beta`` parameter
that controls how strongly the sampling concentrates on higher-ranked demos.
...
CONFIG = {
    "seed": 0,
    "out_dir": ROOT / "task" / "interactive" / "set" / "layer_stats_beta",
    "base_num_pred": 16,
    "sweep_mid_pred": [256],
    "sweep_beta": [0.0, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0, float("inf")],
...
def run_study(
    ...
    sweep_beta = [float(v) for v in cfg["sweep_beta"]]
    ...
    # Accumulator key: (mid_pred, beta, n_demos, step_idx)
        ...
        for bank_idx in range(n_banks):
            ...
            for step_idx in step_indices:
                for prompt_idx in range(n_prompts):
                    ...
                    if result is None:
                        for beta in sweep_beta:
                            for n_demos in exact_n_demos_values:
                                key: AccKey = (mid_pred, beta, n_demos, step_idx)
                    ...
                    # Draw one demo_rng seed per beta upfront so that
                    # the prompt_rng state is deterministic.
                    beta_demo_seeds = [
                        int(prompt_rng.integers(0, np.iinfo(np.int64).max))
                        for _ in sweep_beta
                    ]

                    for beta_idx, beta in enumerate(sweep_beta):
                        demo_rng = np.random.default_rng(
                            beta_demo_seeds[beta_idx]
                        )
                        for n_demos in exact_n_demos_values:
                            key = (mid_pred, beta, n_demos, step_idx)
    ...
    for key in sorted(p_demo_good_acc.keys()):
        mid_pred, beta, n_demos, step_idx = key
...
def _beta_str(beta: float) -> str:
    """Convert a beta value to a display string (handles inf)."""
    if np.isinf(beta):
        return "inf"
    if beta == int(beta):
        return str(int(beta)) if beta >= 1 else f"{beta:.1f}"
    return f"{beta:g}"


def _plot_demo_good_heatmaps(summary_df: pd.DataFrame, out_dir: Path) -> None:
    """Heatmaps of p_demo_good: beta (columns) x n_demos (rows), per (mid_pred, step_idx)."""
    if summary_df.empty:
        return
    required = {"mid_pred", "beta", "n_demos", "step_idx", "p_demo_good"}
    ...
    for (mid_pred, step_idx), part in summary_df.groupby(
        ...
        # Use string labels for beta so inf displays correctly
        part = part.copy()
        part["beta_str"] = part["beta"].map(_beta_str)
        # Preserve numeric ordering via a categorical
        beta_order = [_beta_str(b) for b in sorted(part["beta"].unique())]
        part["beta_str"] = pd.Categorical(
            part["beta_str"], categories=beta_order, ordered=True,
        )
        heat = (
            part.pivot(index="n_demos", columns="beta_str", values="p_demo_good")
            .sort_index()
        )
        if heat.empty:
            continue
        ...
        plt.title(
            f"P(demo good) heatmap (full_rank, mid_pred={mid_pred}, step={step_idx})"
        )
        plt.xlabel("beta")
        plt.ylabel("n_demos")
        plt.tight_layout()
...
def _plot_demo_good_lines(summary_df: pd.DataFrame, out_dir: Path) -> None:
    """Line plots of p_demo_good: beta on x-axis, lines by n_demos, per (mid_pred, step_idx).

    Includes p_random_good and p_good_is_valid as horizontal dashed reference lines.
    Uses string-typed x-axis labels so that beta=inf displays correctly.
    """
    if summary_df.empty:
        return
    required = {
        "mid_pred", "beta", "n_demos", "step_idx",
    ...
    for (mid_pred, step_idx), part in summary_df.groupby(
        ...
        ordered = part.sort_values(["n_demos", "beta"])
        if ordered.empty:
            continue
        plt.figure(figsize=(8, 5))
        sns.lineplot(
        ...
        # Draw bootstrap CIs as vertical bars
        for _, row in ordered.iterrows():
            if pd.isna(row["p_demo_good"]):
                continue
            x_pos = beta_order.index(_beta_str(row["beta"]))
            plt.vlines(
                x=x_pos,
        ...
        # p_random_good reference line (constant across beta/n_demos)
        ...
        # p_good_is_valid reference line (constant across beta/n_demos)
        ...
        plt.ylim(0.0, 1.05)
        plt.title(
            f"P(demo good) (full_rank, mid_pred={mid_pred}, step={step_idx})"
        )
        plt.xlabel("beta")
...
def _plot_random_good_heatmap(summary_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    # p_random_good is constant across beta/n_demos for a given (mid_pred, step_idx).
...
def _plot_rank_sizes(summary_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    # Collapse beta/n_demos dimensions by averaging
    grouped = (
...
def print_console_summary(summary_df: pd.DataFrame) -> None:
    print("\n=== Mid-pred × Plackett-Luce beta difficulty summary ===")
    if summary_df.empty:
        print("(no rows)")
        return
    cols = [
    ...
    sort_cols = [c for c in ["mid_pred", "step_idx", "beta", "n_demos"] if c in present]
